...
def pair_trading_analysis(data, entry_threshold=2, exit_threshold=0):
    try:
        ...
        coint_result = coint(data["KOSPI"], data["S&P 200"])
...
def main():
    ...
    st.write(f"공적분 p-값: {analysis_results['coint_pvalue']:.4f}")
    if analysis_results['coint_pvalue'] < 0.05:
        st.write("KOSPI와 S&P 200은 공적분 관계에 있어 페어트레이딩에 적합할 수 있습니다.")
    else:
        st.write("KOSPI와 S&P 200은 공적분 관계가 약하여 페어트레이딩에 주의가 필요합니다.")
